...
public class ProjectController {
    ...
    // POST http://localhost:8080/api/projects/{id}/sync
    @PostMapping("/{id}/sync")
    @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN', 'ROLE_COMPANY_ADMIN', 'ROLE_WORKSPACE_OWNER', 'ROLE_MANAGER')")
    public ResponseEntity<Map<String, String>> triggerManualSync(@PathVariable Long id, @RequestHeader("X-User-Id") Long userId) {

        ProjectDTO project = projectService.getProjectById(id);

        // On forge la requête pour RabbitMQ
        SyncRequestMessageDTO syncRequest = SyncRequestMessageDTO.builder()
                .projectId(project.getId())
                .repoUrl(project.getUrl())
                .userId(userId)
                .platform(project.getPlatform().toUpperCase())
                .projectName(project.getName())
                .build();

        rabbitMQProducer.sendRepoInitRequest(syncRequest);

        return ResponseEntity.ok(Map.of("message", "Synchronisation lancée en arrière-plan."));
    }

    @GetMapping("/{projectId}/commits/{hash}/diff")
   @PreAuthorize("hasAnyAuthority('ROLE_SUPER_ADMIN', 'ROLE_COMPANY_ADMIN', 'ROLE_WORKSPACE_OWNER', 'ROLE_MANAGER', 'ROLE_DEVELOPER')")
   public ResponseEntity<Map<String, String>> getCommitDiff(@PathVariable Long projectId, @PathVariable String hash) {
       String diff = projectService.getCommitDiff(projectId, hash);
