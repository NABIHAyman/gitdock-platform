...
builder.Services.AddAuthentication("Gateway")
    .AddScheme<GatewayAuthenticationOptions, GatewayAuthenticationHandler>("Gateway", null);
...
builder.Services.AddMassTransit(x =>
{
    ...
    x.UsingRabbitMq((context, cfg) =>
    {
        ...
        cfg.ReceiveEndpoint("task-completed-queue", e =>
        {
            e.Bind("task.exchange", s =>
            {
                s.RoutingKey = "task.completed";
                s.ExchangeType = "topic";
            });
            e.ConfigureConsumer<TaskCompletedConsumer>(context);
        });
    });
});
