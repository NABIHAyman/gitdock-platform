...
public class ProjectServiceImpl implements IProjectService {
    ...
    @Override
    @Cacheable(
            value = CacheConfig.CACHE_COMMITS,
            key = "#projectId + ':b:' + #branchId + ':p:' + #page + ':s:' + #size"
    )
    ...
    // --- Extraction depuis les Headers injectés par la Gateway ---

    private String getRawJwt() {
        // Gardé intact car Feign en a besoin pour transférer la requête à Auth
    ...
    private Long getCurrentUserId() {
        ...
        return (userId != null && !userId.isBlank() && !"null".equals(userId)) ? Long.valueOf(userId) : null;
    }

    private Long getCurrentCompanyId() {
        String compId = request.getHeader("X-Company-Id");
        return (compId != null && !compId.isBlank() && !"null".equals(compId)) ? Long.valueOf(compId) : null;
    }
    ...
    private String getCurrentSubscriptionPlan() {
        // En mode Gateway centralisée, si tu as besoin du plan,
        // la Gateway devra l'injecter dans un header "X-User-Plan".
        // Pour l'instant on met "FREE" en dur ou on lit le header.
        String plan = request.getHeader("X-User-Plan");
        return (plan != null && !plan.isBlank() && !"null".equals(plan)) ? plan : "FREE";
    }

    // 👇 LA LOGIQUE DE LAZY LOADING DU DIFF
   @Override
   public String getCommitDiff(Long projectId, String hash) {
       Project project = projectRepository.findById(projectId)
               .orElseThrow(() -> new RuntimeException("Projet introuvable"));
       verifyProjectAccess(project);

       if (!"GITHUB".equalsIgnoreCase(project.getPlatform().name())) {
           return "L'affichage du diff n'est supporté que pour GitHub actuellement.";
       }

       String token = null;
       try {
           // On récupère le token du manager pour accéder aux repos privés
           token = authServiceClient.getGithubToken("Bearer " + getRawJwt(), project.getManagedById());
       } catch (Exception e) {
           log.warn("⚠️ Aucun token trouvé pour le diff du projet {}", projectId);
       }

       String cleanUrl = project.getUrl().replace("https://github.com/", "").replace("http://github.com/", "").replace(".git", "");
       ...
       try {
           ...
           return restClient.get()
                   .uri("https://api.github.com/repos/{owner}/{repo}/commits/{sha}", parts[0], parts[1], hash)
                   .headers(h -> {
                       if (finalToken != null) h.setBearerAuth(finalToken);
                       // 💥 LE HEADER MAGIQUE DE GITHUB POUR AVOIR LE CODE 💥
                       h.set("Accept", "application/vnd.github.v3.diff");
                   })
                   .retrieve()
                   .body(String.class);
       } catch (Exception e) {
           log.error("❌ Erreur récupération du diff GitHub : {}", e.getMessage());
           return "Impossible de charger le code source depuis GitHub.";
       }
   }
}
