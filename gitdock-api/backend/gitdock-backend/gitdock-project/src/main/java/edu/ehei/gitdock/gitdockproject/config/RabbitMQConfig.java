...
public class RabbitMQConfig {
    ...
    public static final String QUEUE_SENTINEL_RESULT = "gitdock.sentinel.result.queue";
    public static final String ROUTING_KEY_SENTINEL_RESULT = "sentinel.audit.result";
    ...
    public MessageConverter jsonMessageConverter() {
        ...
        idClassMapping.put(
                "edu.ehei.gitdock.gitdocksync.dto.SyncResultMessageDTO",
                SyncResultMessageDTO.class
        );
        // Sentinel envoie ses résultats audit via RabbitMQ en JSON brut Python —
        // pas de __TypeId__ header donc pas besoin de mapping ici

        idClassMapping.put(
                "edu.ehei.gitdock.gitdocksync.dto.CommitEnrichmentRequestDTO",
                edu.ehei.gitdock.gitdockproject.dto.CommitEnrichmentRequestDTO.class // Adapte le package selon le service !
        );
        idClassMapping.put(
                "edu.ehei.gitdock.gitdockproject.dto.CommitEnrichmentRequestDTO",
                edu.ehei.gitdock.gitdockproject.dto.CommitEnrichmentRequestDTO.class
        );
        idClassMapping.put(
                "edu.ehei.gitdock.gitdocksync.dto.CommitEnrichmentResultDTO",
                edu.ehei.gitdock.gitdockproject.dto.CommitEnrichmentResultDTO.class
        );
        idClassMapping.put(
    ...
    @Bean
    public Queue sentinelResultQueue() {
        return new Queue(QUEUE_SENTINEL_RESULT, true); // true = durable
    }

    @Bean
    public Binding bindingSentinelResult(Queue sentinelResultQueue, TopicExchange gitdockExchange) {
        return BindingBuilder.bind(sentinelResultQueue).to(gitdockExchange).with(ROUTING_KEY_SENTINEL_RESULT);
    }

    @Bean
    public Queue enrichRequestQueue() { return new Queue(QUEUE_ENRICH_REQUEST, true); }
    @Bean
    public Queue enrichResultQueue() { return new Queue(QUEUE_ENRICH_RESULT, true); }

    @Bean
    public Binding bindingEnrichReq(Queue enrichRequestQueue, TopicExchange gitdockExchange) {
        return BindingBuilder.bind(enrichRequestQueue).to(gitdockExchange).with(ROUTING_KEY_ENRICH_REQUEST);
    }
    @Bean
    public Binding bindingEnrichRes(Queue enrichResultQueue, TopicExchange gitdockExchange) {
