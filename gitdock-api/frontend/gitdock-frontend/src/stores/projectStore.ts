...
export const useProjectStore = defineStore('project', () => {
    ...
    const fetchProjectBranches = async (projectId: number | string) => {
       // Ne déclencher le gros spinner QUE si c'est un nouveau projet
       const isNewProject = currentProject.value?.id !== Number(projectId);
       if (isNewProject) {
            isLoading.value = true;
        }
    ...
    const fetchBranchCommits = async (
        ...
        if (page === 0) {
            isLoading.value = true
            commits.value = []
            // On s'assure de réactiver le flag quand on recharge de zéro
        ...
        try {
            ...
            // Sécurité : On s'assure que newCommits est toujours un tableau (évite les crashs du spread operator)
            const newCommits = response.content || []
            ...
            // Le flag magique de Spring Boot qui dit s'il reste des pages après
    ...
    const triggerManualSync = async (projectId: number | string) => {
        isSyncing.value = true
        try {
            await projectService.triggerManualSync(projectId)
            useNotificationStore().info('Synchronisation lancée en arrière-plan. Vous serez notifié.')
        } catch (error: unknown) {
            isSyncing.value = false;
            const axiosError = error as { response?: { data?: { message?: string } } }
            useNotificationStore().error(axiosError.response?.data?.message ?? "Erreur lors du lancement de la synchronisation")
        }
    }
