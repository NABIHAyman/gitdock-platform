...
const saveBadge = async () => {
  ...
  try {
    ...
    if (isEditing.value && currentBadgeId.value) {
      await badgeService.update(currentBadgeId.value, badgeData)
      showNotify("Badge repository updated successfully.")
    } else {
      await badgeService.create(badgeData)
...
const confirmDeleteBadge = async () => {
  ...
  if (badgeToDeleteId.value) {
    try {
      await badgeService.delete(badgeToDeleteId.value)
