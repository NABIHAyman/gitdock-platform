...
import { TaskService, type TaskFormDataDTO, type TaskListItemDTO } from '@/services/TaskService'
  ...
  try {
    ...
    formData.value = {
      ...
      assignee: task.assignedTo
        ? teamMembers.value.find(u => u.id === Number(task.assignedTo))
        : null,
      epic: task.epic ? epics.value.find(e => e.id === Number(task.epic)) : null,
      part: task.part ? parts.value.find(p => p.id === Number(task.part)) : null,
      level: task.level ? levels.value.find(l => l.id === Number(task.level)) : null
    }

    taskInfo.value = task as any
  } catch (error) {
